...
router.post("/", protect, async (req, res) => {
  try {
    ...
    if (restaurantRating) {
      ...
      const allRestReviews = await Review.find({ restaurantId, reviewType: "restaurant" }).select("rating").lean();
    ...
    if (foodItemReviews && Array.isArray(foodItemReviews) && foodItemReviews.length > 0) {
      for (const itemReview of foodItemReviews) {
        ...
        if (foodItem) {
          const allFoodReviews = await Review.find({ foodItemId, reviewType: "food_item" }).select("rating").lean();
    ...
    // Invalidate caches
    await clearPatternCache('cache:/api/reviews*');
    await clearPatternCache('cache:/api/restaurant/public*');
...
router.get("/restaurant/:restaurantId", cacheResponse(300), async (req, res) => {
  try {
...
router.get("/food/:foodItemId", cacheResponse(300), async (req, res) => {
  try {
...
router.get("/restaurant/:restaurantId/food-items", cacheResponse(300), async (req, res) => {
  try {
    const { restaurantId } = req.params;
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
