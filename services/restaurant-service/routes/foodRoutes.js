...
router.get("/", cacheResponse(600), async (req, res) => {
  try {
    ...
    const foods = await FoodItem.find({})
      .select("name description price category imageUrl isAvailable isVeg averageRating restaurantId tags")
...
router.get("/search", async (req, res) => {
  try {
    ...
    const foods = await FoodItem.find(dbQuery)
      .select("name description price category imageUrl isAvailable isVeg averageRating restaurantId tags")
      .populate('restaurantId', 'name profileImage cuisineType address isOpen')
      .skip(skip)
      .limit(limit)
      .lean();
...
router.get("/restaurant/:restaurantId", cacheResponse(600), async (req, res) => {
  try {
    ...
    .select("name description price category imageUrl isAvailable isVeg averageRating tags")
    .sort({ createdAt: -1 })
    .lean();
...
router.post("/", protect, upload.single('image'), async (req, res) => {
  try {
    try { fs.appendFileSync("request_logs.txt", `[${new Date().toISOString()}] POST /foods Req Body: ${JSON.stringify(req.body)} | File: ${req.file ? req.file.path : 'None'}\n`); } catch(e){}
    ...
    if (req.file) {
      ...
      try { fs.appendFileSync("request_logs.txt", `[${new Date().toISOString()}] Fail: Base64\n`); } catch(e){}
      return res.status(400).json({ message: "Base64 images are not allowed. Please upload using multipart/form-data." });
    }

    if (!name || !price || !category) {
      try { fs.appendFileSync("request_logs.txt", `[${new Date().toISOString()}] Fail: Missing required: name=${name}, price=${price}, category=${category}\n`); } catch(e){}
    ...
    if ([name, description, category].some(field => field && /[<>]/.test(field))) {
      try { fs.appendFileSync("request_logs.txt", `[${new Date().toISOString()}] Fail: HTML tags\n`); } catch(e){}
...
router.get("/my-foods", protect, async (req, res) => {
  try {
    ...
    const foods = await FoodItem.find({ restaurantId: req.user._id })
